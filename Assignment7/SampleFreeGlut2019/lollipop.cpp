...
const char *WINDOWTITLE = { "Assignment 1 -- Justin Boyer" };
...
InitLists( )
{
	...
	//heads
		...
		g = 1.;
		direction = false;
		const float handleRad = 0.02;
		for (int i = 0; i < NUMSEGS; i++)
		{
			glColor3f(r, g, b);
			glVertex3f(handleRad * sin(ang), z1,  handleRad * cos(ang));
			glVertex3f(handleRad * sin(ang), z2, handleRad * cos(ang));
			ang += dang;
		}
		glEnd();
